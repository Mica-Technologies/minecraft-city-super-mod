...
| P8 | Mount kit and cover: neighbour scan cached on the tile entity, boxes in a shared list per exact shape; the server's `neighborChanged` and a head's own sync drop the cache at once | `421409f1d` + (this change) | 10.9 / 4.8 µs beside a head | 1.1 / 1.1 µs. Frames identical; a head placed or tilted beside one shows within 250 ms |
| - | `resetColor()` after replaying a vertex-coloured shared list (emergency light) | (this change) | GlStateManager could skip the next `color(1,1,1,1)` | cache matches GL again |
