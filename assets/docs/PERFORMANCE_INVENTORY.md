...
| X1 | A sync packet rebuilds the chunk section only when something a baked model reads changed (`AbstractTileEntity.getBakedModelKey`); heads key on tilt (the backplate reads it), crosswalks, thermostats, lane control, blankout, sensor, speaker, TTS and speed limit signs on nothing | `b39b31be9` | thermostat flood in a 64-crate section: max frame 445 ms (doc); same-session control on a street sign 2,250 ms | max 12.2 ms against 11.7 quiet |
| X2 | Crosswalk, blankout and lane control set `dirty` only when an appearance field changed, not on every packet (the countdown once a second, an aspect change) | `b39b31be9` | three list recompiles per crosswalk a second in clearance; a body recompile per aspect change | none; a synced colour change still redraws (checked on all three) |
| P1 | Portable message sign: trailer, outriggers, mast, panel, solar panel and flasher housings in one shared list keyed on light, trailer colour and housings fitted; text and lenses live | `98f6728fa` | 125 µs a sign (flashers off) | 2.4 µs; 256 signs 33 to 1.5 ms. Pixels identical |
| P1 | Portable speed limit sign, the same way (LED screen baked fullbright) | `1c5a4c8bb` | 127 µs (flashers off), 21 (flashers none) | 6.5 / 5.5 µs. Pixels identical in both modes |
| P2 | School zone beacon: body and housings in one shared list, the six-line legend in a second keyed on the speed limit (`CsmFontRenderer.addString`); bulbs live | `9abac992e` | 75 µs idle, 128 flashing | 7.2 / 7.4 µs; 256 flashing 33.6 to 2.7 ms. Pixels identical (idle) |
| P6 | Radar speed sign: rounded panels static, panel and window in a shared list; text live | `fa54ba666` | 20 µs | 4.6 µs. Pixels identical |
| P7 | Railroad crossing gate: arm, counterweight and lamps in a shared list per look (dark, first half lit, second half lit) under the swing rotation; one block-state read a frame, bounding box cached | `1cf18288d` | 23 µs (gate 3) | 1.1 µs. Pixels identical unpowered; powered, both lamp looks match live exactly |
| P5 | Arrow board: body and lamp grid per (pattern, stage, light), halos per (pattern, stage); the raw structure map folded into the shared store. The 30 to 34 µs "regression" was `6bb3a0d18` turning the halos on away from the world origin, as intended | `fa5181b9f` | 34 µs | 1.3 µs; 256 boards 9.6 to 1.1 ms. Animates, so compared by same-path spread only |
| P8 | Mount kit and cover: neighbour scan cached on the tile entity, boxes in a shared list per exact shape; the server's `neighborChanged` and a head's own sync drop the cache at once | `421409f1d` + `6b8781a64` | 10.9 / 4.8 µs beside a head | 1.1 / 1.1 µs. Frames identical; a head placed or tilted beside one shows within 250 ms |
| - | `resetColor()` after replaying a vertex-coloured shared list (emergency light) | `6b8781a64` | GlStateManager could skip the next `color(1,1,1,1)` | cache matches GL again |
| S3 | Thermostats: strings, their widths and the outside temperature cached per thermostat (they were single fields on the shared renderer, so thermostats in view overwrote each other every frame); time reformatted per in-game minute, not per tick; biome read once a second | (this change) | 8.8-9.8 µs | 7.7-8.9 µs (cross-session). The rest is vanilla `drawString`, which cannot go in a list: the arrows are on a Unicode page, a second texture |
| X5 | HVAC HUD: one walk over the nearby chunks' tile entities answers both "near any HVAC" and "which sources" (`getTemperatureAt(world, pos, radius, out)`), instead of a 9-chunk walk and then a 49-chunk one every half second. The inventory's version (skip the temperature when not near) would have changed the smoother's entry transient, so it was not taken | (this change) | 58 chunk walks / 0.5 s | 49; answers identical. HUD checked near and away from a heater |
| X1 | Radar speed sign and school zone beacon join the sync opt-out (the radar syncs up to five times a second with a vehicle in its zone) | (this change) | section rebuild per sync | none |
| - | Tattle-tale beacon marks its tile entity dirty when its mode is cycled, so the mode is saved | (this change) | could be lost on reload | saved |
| - | Crane head: lists released on disconnect (they are not in a `CsmDisplayListCache`), `resetColor()` after its vertex-coloured list | (this change) | leaked into the next world | released |
...
A fix's A/B/A runs in one session behind `/csm renderpass skip sharedBakesPerFrame` (or the
renderer's own toggle); the scratch harness for that is `ab.py`, kept beside the others.
