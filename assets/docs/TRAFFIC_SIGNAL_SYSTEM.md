...
| Body style | `TrafficSignalBodyStyle` | Standard (flat back) / Bubbled (Eagle-style rounded casting) / PV (the deep housing of an optically programmed head: a full-size frame plate behind the door, then a box inset 0.7 on every side and as deep as the section is tall, so each section is its own casting with a gap to the next; finely ridged sides, chamfered rear, reaching 7 units into the block behind). Per-section; swaps only the housing geometry (`TrafficSignalVertexData.resolveBodyData`, plus `resolveBodyShadeData` for the parts drawn a shade darker so recesses read without directional lighting) — doors, visors and bulbs are shared. The mounts bolt to the deepest section's rear (`bodyRearZ`), and a bracket on a bubbled or PV end sinks into the housing (1.5 and 0.9 units) so it meets the dome or the inset box rather than starting in the air behind the frame |
