...
| Body style | `TrafficSignalBodyStyle` | Standard (flat back) / Bubbled (Eagle-style rounded casting) / PV (the deep housing of an optically programmed head: a full-size frame plate behind the door, then a box inset 0.7 on every side and as deep as the section is tall, so each section is its own casting with a gap to the next; finely ridged sides, chamfered rear, reaching 7 units into the block behind, and drawn under the visor's 9° shear about the door plane so the whole optical unit aims down together). Per-section; swaps only the housing geometry (`TrafficSignalVertexData.resolveBodyData`, plus `resolveBodyShadeData` for the parts drawn a shade darker so recesses read without directional lighting) — doors, visors and bulbs are shared. PV sections are joined through coupling hubs in the gaps (collar plus flanges, at the box's mid-depth, in the mount colour), with the same hub as a boss on an end section's outer face. A head that is PV throughout anchors its brackets at that mid-depth so the stub rises out of the boss; otherwise the mounts bolt to the deepest section's rear (`bodyRearZ`). A bracket on a bubbled or PV end sinks into the housing (1.5 and 0.9 units) so it meets the dome or the inset box rather than starting in the air behind the frame. A rear mount on a head whose housing reaches past the block's back face (`bodyMaxZ` > 16: the PV box, the bubbled belly) aims its arms at the pole two blocks behind, since the housing occupies the first |
...
| Alternate flash | `TrafficSignalFlashPattern` | `OFF` (lit on the second half-second) / `B` (lit on the first half-second, the wig-wag counterpart of `OFF`) / `C` (rapid five-pulse strobe, shares its timing with the Barlo safety beam) / `D` (the same strobe delayed by one burst length, C's wig-wag counterpart) / `E` and `F` (the `OFF`/`B` pair at 5 Hz instead of 1 Hz: 100 ms lit, 100 ms dark, `E` lit on the second half of each cycle). The 1300 ms strobe cycle holds C's burst at 0-500 ms and D's at 500-1000 ms, so a pair set to C and D alternate and never overlap; the fast cycle divides the slow one, so E and F line up with OFF and B at the top of each second. Persisted as an ordinal under `flsP`; the pre-pattern boolean `altF` migrates to `OFF`/`B` |
...
`DEFAULT` (raw cast aluminum, body ~0.64), `WHITE`, `DARK_GRAY`, `BLACK`, `ORANGE` (the portable
signal trailers' own highway orange, for a bracket clamped to one of their booms — it is painted
with the machine, not left in raw aluminum). Cycled by sneak + right-click and persisted on the
tile entity **by ordinal**, which is why `ORANGE` is appended rather than slotted in beside the
other colors: anywhere else and every mount kit already placed silently repaints itself.

#### Clamped to a portable signal trailer's boom

A mount kit placed in one of the cells a signal trailer's boom runs through clamps to the boom
instead of to a pole, which is what lets a head sit **beside** the boom in the traditional way
rather than hanging under it with `SignalHeadMountType.OVERHEAD`.

The bracket is **shallower** in that case and its back moves **forward**. Everywhere else, the
thing it clamps is a pole in the block *behind*, so the spine, the pivot hubs and the far end of
the arms' webs all sit at the back face of the block. A boom is not there — it is a bar through
the **middle** of this block — so a bracket left at its normal depth reaches straight past it and
clamps thin air. `SignalInfo.backZ` carries that depth through every piece; it is
`DEFAULT_BACK_Z` normally and the boom's near face when clamped. The mounting collar, which
wraps a vertical pole rising past the top of the bracket, is replaced by a **saddle** band around
the bar, because on a boom there is no such pole.

Finding the boom is the awkward part and is why `SignalTrailerBoom` exists. The boom is the one
thing in the mod a mount can fix to that **is not a block** — it is geometry belonging to a
`portable_signal_trailer_arm` up to eight cells away — so the kit cannot find it by looking at its
neighbours. It walks back along both axes across its own facing (a head on a boom faces *across*
it) looking for the trailer that owns it. The cell layout it walks comes from
`SignalTrailerGeometry`, generated by `gen_work_zone_devices.py` from the same numbers the boom is
swept from, so the clamp cannot drift off the bar when the boom is next reshaped.

Two things that are easy to get wrong here:

- The result is cached on the tile entity but **expires on a timer**, not only on
  `invalidateCachedBB()`. The trailer is eight cells out and eight down, so placing or breaking
  it never reaches the mount kit as a neighbour change; a kit put up before its trailer would
  otherwise cache "no boom" and keep clamping air.
- The trailer **settles** onto the road under it, and the whole model moves with that, boom
  included. `SignalTrailerBoom.settleY` carries the offset so the saddle follows the bar down a
  slope instead of floating over it.
