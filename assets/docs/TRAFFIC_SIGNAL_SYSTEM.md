...
| Alternate flash | `TrafficSignalFlashPattern` | `OFF` (lit on the second half-second) / `B` (lit on the first half-second, the wig-wag counterpart of `OFF`) / `C` (rapid five-pulse strobe, shares its timing with the Barlo safety beam) / `D` (the same strobe delayed by one burst length, C's wig-wag counterpart). The 1300 ms strobe cycle holds C's burst at 0-500 ms and D's at 500-1000 ms, so a pair set to C and D alternate and never overlap. Persisted as an ordinal under `flsP`; the pre-pattern boolean `altF` migrates to `OFF`/`B` |
