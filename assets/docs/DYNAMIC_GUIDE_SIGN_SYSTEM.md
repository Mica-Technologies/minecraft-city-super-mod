...
| `GuideSignShieldType` | Route shield marker | 67 values: 8 generic (INTERSTATE, INTERSTATE_BUSINESS, US_ROUTE, STATE_SQUARE, STATE_CIRCLE, COUNTY_ROUTE, TOLL, BLANK_CUSTOM), the 50 states, DC, 6 Canadian provinces (ON, QC, NB, NS, NL, PE) and the two Alto markers — **append-only: ordinals are serialized**. Each carries its `(atlasCol, atlasRow)` and where its route number goes: colour, cap height, maximum width and centre (see Rendering Proportions). |
...
- Atlas is **512×1024**, divided into **64×64** cells → 8 columns × 16 rows.
...
- **Row 1:** CA, TX, FL, NY, CT, MA, ME, NH. **Row 2:** RI, VT, AL, AK, AZ, AR, CO, DE.
  **Row 3:** GA, HI, ID, IL, IN, IA, KS, KY.
- **Rows 4–5, cols 0–4:** directional arrows (white on transparent).
- **Row 6:** LA, MD, MI, MN, MS, MO, MT, NE. **Row 7:** NV, NJ, NM, NC, ND, OH, OK, OR.
  **Row 8:** PA, SC, SD, TN, UT, VA, WA, WV. **Row 9:** WI, WY, DC, ON, QC, NB, NS, NL.
- **Row 10:** PE, then the Alto markers (2- and 3-digit variants) in cols 1–4.
- **Rows 11–12:** the dynamic street sign's civic logos.

Shield backgrounds carry **no baked text** — the TESR sets the route number over the shield, so one
background serves every route number.
...
512×1024 ARGB image. Every route marker except State Square, Toll and Blank/Custom (drawn
programmatically) is an SVG from `dev-env-utils/src/main/resources/guidesign/shields/`, rendered
through Apache Batik by `drawSvgShield(g, col, row, file)` into the cell with a 2 px margin, centred
and at its own aspect (Tennessee's and South Carolina's 5:4 markers letterbox; Ontario's tall crown
shield pillarboxes). Arrows are drawn programmatically in all 10 directions.

**State, DC and province markers are the real markers,** not lookalikes. Each is a public-domain
route marker from Wikimedia Commons, simplified to its blank face and named after its enum constant
(`california.svg`, `new_brunswick.svg`, ...); `SOURCES.md` beside them records each file's source,
author and licence. They keep what the real sign has: the black square behind an outline
(Alabama, Georgia), the fixed wording (TEXAS, ILLINOIS, NORTH DAKOTA), the emblems (Colorado's
flag, Nebraska's wagon, Wyoming's bucking horse), and the real colours (California's green spade,
Minnesota's blue, Vermont's green frame). Delaware, Iowa, Kentucky, Mississippi and New Jersey post
the plain MUTCD M1-5 circle, so they share `state_circle.svg` with the generic circle.

They used to be drawn from memory — `drawStateShield` filled a hand-coded silhouette, eleven of them
bespoke and the rest a circle, hexagon, diamond, octagon or oval in a "state-themed" colour — and
that is how California's miner's spade came out as a map pin (#198). Nobody can check a
silhouette typed as coordinates; a sourced SVG is checked by looking at it. Don't bring the
programmatic shapes back for a new marker: find its blank on Commons (the category "Highway shield
blanks of the United States" / "... of Canada" has most of them), and keep only files marked public
domain.

**Where the number goes is measured, not typed.** A real marker does not set its number in the
middle: Texas sets it above the word TEXAS, Colorado below the flag, Idaho in the corner the
state's outline leaves free. `dev-env-utils/scripts/measure_shield_legends.py` reads each marker's
cell off the regenerated atlas, takes the connected region of the face under a seed point (bridging
thin outline strokes the real sign prints the number across, like Florida's and Oklahoma's state
outlines), fits the largest two-digit number inside it with a margin, and writes the four
placement values and the colour into `GuideSignShieldType`. Its `--check` exits 1 when the enum
and the atlas disagree. The seed points and colours live in the script's `MARKERS` table and were
taken from each state's numbered marker on Commons.
...
| `GuideSignShieldType.getRouteTextCapFraction()` | `0.42` generic; per marker `0.24`–`0.42` | Route-number cap height, as a fraction of `SHIELD_SIZE` (`DEFAULT_ROUTE_CAP_FRACTION` for the generic shields). A state marker's is the height of the largest two-digit number that fits its number area, so one- and two-digit routes are set at the same height, as on the real sign. |
| `GuideSignShieldType.getRouteTextMaxFraction()` | per shield, `0.30`–`0.77` | Max width the route number may span (as a fraction of the shield's width) before it shrinks to fit. The generic shields' values were measured from the atlas cell's y=26–38 mid-band (narrowest opaque run, ×0.80, clamped to `[0.30, 0.62]`). A state marker's is the width of that largest two-digit number, so a three-digit route shrinks to the same width and stays inside the outline. |
| `GuideSignShieldType.getRouteTextCenterX()` / `getRouteTextCenterY()` | `0.5` / `0.5` generic; per marker | Centre of the route number as fractions of the shield, measured from the cell's **top**-left. Pixel space runs upwards, so the renderers subtract the Y offset: `shieldCenterY − (centerY − 0.5) × SHIELD_SIZE`. Colour comes from `getRouteTextColor()` (the real sign's numeral colour: black on white, white on California's green and Minnesota's blue, green on Vermont, blue on South Carolina, yellow on the county pentagon). The state markers' four values and colour are written by `measure_shield_legends.py`; don't edit them by hand. |
...
| State markers that were not the state's marker: California a map pin; Texas a map of Texas when the real marker is a square; most states a generic hexagon or diamond | The silhouettes were typed from memory as coordinates, which nobody can review | Every state, DC and province marker is a sourced public-domain SVG (`guidesign/shields/SOURCES.md`), and its number placement is measured off the atlas by `measure_shield_legends.py`. |
...
1. **Append** an enum entry to `GuideSignShieldType` (never insert: ordinals are serialized) with a
   free `(atlasCol, atlasRow)` cell — row 10 cols 5–7 and rows 13–15 are unused.
2. Find the marker's public-domain blank on Wikimedia Commons, simplify it (strip editor metadata,
   empty text placeholders and any numeral; keep a `viewBox`), save it as
   `guidesign/shields/<constant_lower>.svg` and add its row to `SOURCES.md`.
3. Add a `drawSvgShield(g, col, row, "<file>.svg")` line to `drawShields` and regenerate
   `sign_atlas.png` with `GuideSignAtlasTool`.
4. Add the constant to `MARKERS` in `dev-env-utils/scripts/measure_shield_legends.py` with a seed
   point on the face where the real sign sets its number and the numeral colour, run it, and confirm
   `--check` passes.

The TESR, the street sign and the GUI pick it up automatically — shields are looked up by atlas
coordinate and the GUI cycles `GuideSignShieldType.values()`.
