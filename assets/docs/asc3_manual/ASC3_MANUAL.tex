...
\begin{tikzpicture}[line cap=round,
  thru/.style={-{Stealth[length=3mm]},line width=1.4mm},
  turn/.style={-{Stealth[length=2.6mm]},line width=1.0mm}]
  % roads
  \fill[asphalt!30] (-4.6,-1.1) rectangle (4.6,1.1);
  \fill[asphalt!30] (-1.1,-3.4) rectangle (1.1,3.4);
  \draw[white,dashed,line width=0.5pt] (-4.6,0) -- (-1.1,0);
  \draw[white,dashed,line width=0.5pt] (1.1,0) -- (4.6,0);
  \draw[white,dashed,line width=0.5pt] (0,-3.4) -- (0,-1.1);
  \draw[white,dashed,line width=0.5pt] (0,1.1) -- (0,3.4);
  % EB through phi2 + EB left phi1
  \draw[thru,sigGreen] (-4.2,-0.55) -- (-1.6,-0.55);
  \node[below,font=\small\bfseries,color=sigGreen!80!black] at (-2.9,-0.72) {\ph{2} EB through};
  \draw[turn,sigGreen!75!black] (-2.4,-0.3) to[out=0,in=-90] (-0.55,1.6);
  \node[font=\small\bfseries,color=sigGreen!60!black] at (-2.35,0.62) {\ph{5}};
  % WB through phi6 + WB left phi5
  \draw[thru,sigGreen] (4.2,0.55) -- (1.6,0.55);
  \node[above,font=\small\bfseries,color=sigGreen!80!black] at (2.9,0.72) {\ph{6} WB through};
  \draw[turn,sigGreen!75!black] (2.4,0.3) to[out=180,in=90] (0.55,-1.6);
  \node[font=\small\bfseries,color=sigGreen!60!black] at (2.35,-0.62) {\ph{1}};
  % NB through phi4 + NB left phi3
  \draw[thru,sigYellow!80!black] (0.55,-3.1) -- (0.55,-1.5);
  \node[right,font=\small\bfseries,color=sigYellow!60!black] at (0.72,-2.4) {\ph{4}};
  \draw[turn,sigYellow!65!black] (0.3,-2.0) to[out=90,in=0] (-1.5,-0.55);
  \node[font=\small\bfseries,color=sigYellow!50!black] at (-0.8,-1.7) {\ph{7}};
  % SB through phi8 + SB left phi7
  \draw[thru,sigYellow!80!black] (-0.55,3.1) -- (-0.55,1.5);
  \node[left,font=\small\bfseries,color=sigYellow!60!black] at (-0.72,2.4) {\ph{8}};
  \draw[turn,sigYellow!65!black] (-0.3,2.0) to[out=-90,in=180] (1.5,0.55);
  \node[font=\small\bfseries,color=sigYellow!50!black] at (0.8,1.7) {\ph{3}};
\end{tikzpicture}
\caption{NEMA phase numbering as used by \key{Load Std 8-Phase}. Green = main
street (\ph{1},\ph{2},\ph{5},\ph{6}); yellow = side street
(\ph{3},\ph{4},\ph{7},\ph{8}). Even = through, odd = the paired protected left.}
\label{fig:nema}
\end{figure}
...
\end{tikzpicture}
\caption{The dual-ring, dual-barrier structure. One green per ring; any ring-1
phase may run beside any ring-2 phase \emph{in the same group} (\ph{1}+\ph{5},
\ph{1}+\ph{6}, \ph{2}+\ph{5}, \ph{2}+\ph{6}, \ldots). Rings cross a barrier only
together.}
...
\begin{description}[nosep,leftmargin=2.6cm,style=nextline]
  ...
  \item[Max Green] the ceiling while a conflicting call waits: reaching it is a
        \emph{max-out} and the green ends even with traffic still flowing.
...
companion holds green with the called phase and clears when it clears
(Section~\ref{sec:dualentry} covers the timing). A real call always beats dual
entry, and DE picks the \emph{first} active DE phase in ring order — flag only the
phase you want as the companion.
...
with both its own through phase \emph{and} the compatible cross-street left.
Managed like preempts (\key{P+}/\key{P−}, \key{Prev}/\key{Next}):
...
\field{INCL}    & The included phases: green with any of them, yellow during their
                  clearance, red otherwise. \\
...
same moment as the called phase, holds green with it, and clears with it; while
...
\midrule
\ph{2} & Main through (EB) & 1 & 50 & \checkmark \\
\ph{4} & Cross through     & 1 & 40 &  \\
\ph{5} & Main left (EB)    & 2 & 15 &  \\
\ph{6} & Main through (WB) & 2 & 35 & \checkmark \\
...
\begin{description}[nosep,leftmargin=4.2cm,style=nextline]
  ...
  \item[Dual entry (DE)] Serving an uncalled companion phase so both rings show
    green on the active barrier; the companion clears with the called phase.
