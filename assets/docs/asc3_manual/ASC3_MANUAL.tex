...
\definecolor{pedBlue}{RGB}{60,110,190}
...
\texttt{FLASH}        & & Yellow/red flash; program which movement gets which
                          on \screen{MAP} (Section~\ref{sec:flashprog}). \\
...
\begin{tikzpicture}[line cap=round,scale=0.92,
  ...
  pedln/.style={{Stealth[length=2.2mm]}-{Stealth[length=2.2mm]},dashed,pedBlue,
                line width=0.6pt},
  ped/.pic={% minimalist walking pedestrian, ~0.42 tall, feet at y=-0.215
    \fill (0,0.135) circle (0.06);
    \draw[line width=0.8pt] (0,0.065) -- (0,-0.08)
        (0,-0.08) -- (-0.075,-0.215) (0,-0.08) -- (0.075,-0.215)
        (-0.08,0.027) -- (0.08,-0.013);
  }]
  ...
  % ---- Pedestrian movements (FHWA style): dashed crosswalks ringing the box.
  % P(n) walks concurrently with through phase n: P2/P6 cross the minor street
  % beside the main-street greens; P4/P8 cross the major street with the side
  % street. Drawn first so the vehicle arrows stay on top.
  \draw[pedln] (-1.55,1.35) -- (1.55,1.35);   % north leg  (P4)
  \draw[pedln] (-1.55,-1.35) -- (1.55,-1.35); % south leg  (P8)
  \draw[pedln] (-1.35,-1.55) -- (-1.35,1.55); % west leg   (P2)
  \draw[pedln] (1.35,-1.55) -- (1.35,1.55);   % east leg   (P6)
  \node[font=\scriptsize\bfseries,color=pedBlue] at (0.95,1.12) {P4};
  \node[font=\scriptsize\bfseries,color=pedBlue] at (-0.95,-1.12) {P8};
  \node[font=\scriptsize\bfseries,color=pedBlue] at (-1.13,-0.6) {P2};
  \node[font=\scriptsize\bfseries,color=pedBlue] at (1.13,0.6) {P6};
  \pic[pedBlue] at (-0.7,1.565) {ped};
  \pic[pedBlue] at (0.7,-1.135) {ped};
  \pic[pedBlue] at (-1.35,0.95) {ped};
  \pic[pedBlue] at (1.35,-0.95) {ped};
  ...
  % legend
  \draw[thru,inkhead!75,line width=1.0mm] (-3.45,-4.6) -- (-2.7,-4.6);
  \node[anchor=west,font=\scriptsize,color=inkhead] at (-2.55,-4.6) {Vehicle movements};
  \draw[pedln] (0.55,-4.6) -- (1.35,-4.6);
  \node[anchor=west,font=\scriptsize,color=inkhead] at (1.5,-4.6) {Pedestrian movements};
\end{tikzpicture}
...
\emph{opposing} through. Pedestrian movements (dashed blue) carry the number of
the through phase they walk with: P2/P6 cross the minor street alongside the
main-street greens, P4/P8 cross the major street with the side-street greens.}
\label{fig:nema}
\end{figure}
...
\textbf{Left's FYA ref} & \textbf{Max green (thru/left)} & \textbf{Flash} \\
\midrule
1st N--S circuit & \ph{2} & \ph{5} & \texttt{P6} & 60\,s / 15\,s & \texttt{YEL}/\texttt{RED} \\
2nd N--S circuit & \ph{6} & \ph{1} & \texttt{P2} & 60\,s / 15\,s & \texttt{YEL}/\texttt{RED} \\
1st E--W circuit & \ph{4} & \ph{7} & \texttt{P8} & 30\,s / 15\,s & \texttt{RED}/\texttt{RED} \\
2nd E--W circuit & \ph{8} & \ph{3} & \texttt{P4} & 30\,s / 15\,s & \texttt{RED}/\texttt{RED} \\
\bottomrule
\end{tabular}
...
through was also assigned. The \textbf{Flash} column is the \field{FLSH} program
written for the through and the left (Section~\ref{sec:flashprog}).}
\label{tab:template}
\end{table}
...
\field{LOCK} & Call memory & \texttt{Lock} = latch vehicle calls until served;
                          ...
                          sees a vehicle). \\
\field{FLSH} & Flash program & What this phase's movement does in flash:
                          \texttt{--} automatic / \texttt{YEL} / \texttt{RED} /
                          \texttt{off}. \texttt{n/a} on a \texttt{PED} phase.
                          Section~\ref{sec:flashprog}. \\
\bottomrule
\end{tabular}
...
companion \emph{rides} with the other ring's barrier service: it holds green
through the other ring's within-barrier transitions (a through clearing to serve
its left, say), and demand waiting on \emph{another} barrier never strips it off
early — under coordination the main-street phases are always calling, and the
side-street companion rides through that and clears \emph{with} its companion
when the barrier's work is done. Only same-barrier demand it conflicts with (a
call in its own ring, or its FYA partner) ends it sooner
(Section~\ref{sec:dualentry} covers the timing). A real call always beats dual
entry, and DE picks the \emph{first} active DE phase in ring order — flag only
the phase you want as the companion.
...
\paragraph{Flash program (FLSH).}\label{sec:flashprog}
Left alone (\texttt{--} on every phase), flash mode uses the controller's
automatic rule, which decides yellow-versus-red from the \emph{order the circuits
were linked in}: the first circuit flashes yellow and the rest flash red on
alternating half-cycles. That lines up at a simple intersection wired in street
order, but rarely at one programmed here — the main street may well be C3, and a
single circuit often carries both a through and a left.

Setting \field{FLSH} on any phase switches the controller to a \emph{programmed}
flash built from this screen:

\begin{itemize}
  \item Each programmed phase drives its own movement's heads — so a through can
        flash \texttt{YEL} while the left on the same circuit flashes \texttt{RED}.
        A through carries its concurrent right turn; a left carries both its
        protected and permissive lenses.
  \item Approaches alternate \emph{by barrier}: barrier~A (\ph{1}, \ph{2},
        \ph{5}, \ph{6} — the main street and its lefts) lights on one half-cycle,
        barrier~B (\ph{3}, \ph{4}, \ph{7}, \ph{8} — the side street) on the
        other. That's the familiar main-versus-cross wig-wag, taken from the
        phase map rather than from the order you linked the circuits in.
  \item Anything you don't program flashes \textbf{red}, on barrier~A's beat.
        Pedestrian heads are dark and beacons flash as they always do.
\end{itemize}

The program applies however the controller reaches flash — the \texttt{FLASH}
mode, nightly fallback, or power-loss fallback. It does not affect the all-red
flash or the fault flash, which stay all-red by design.

\key{Load Std 8-Phase} fills it in for you: main-street throughs \texttt{YEL},
side-street throughs \texttt{RED}, every left \texttt{RED} --- the standard
program at a typical four-way.
...
\field{SIGNS}   & \emph{Circuits} (not phases) whose blankout boxes light for the
                  whole sequence — see \emph{Blankout signs} below. \\
\bottomrule
\end{tabular}
...
\field{SIGNS} works the same way but lists circuits.}
\label{tab:preempt}
\end{table}
...
\paragraph{Blankout signs (SIGNS).}\label{sec:preemptsigns}
A blankout box set to the 	exttt{Train} or 	exttt{No U-Turn} legend has no phase
logic of its own — it is dark until a preempt lights it. Select the circuits its
boxes are linked to in \field{SIGNS}, and every such box on those circuits comes up
for the entire sequence, from the first yellow of ENTER through the end of EXIT: the
	exttt{Train} legend flashes, 	exttt{No U-Turn} burns steady. Both go dark again as
normal service resumes.

That is the whole rail-crossing package in one place: \field{TRACK} flushes the
crossing, \field{DWELL} holds the parallel street, and \field{SIGNS} lights the TRAIN
signs on the approaches for as long as the gates are down.

The other two legends are unaffected: 	exttt{No Left Turn} / 	exttt{No Right Turn}
boxes always follow the turns the current phase permits, and 	exttt{Don't Walk} /
	exttt{Do Not Enter} boxes link as ordinary pedestrian heads. A box that isn't linked
to the controller at all stays fully manual — set its state with the signal head
config tool or the box's own configuration screen.
...
\begin{itemize}[nosep]
  \item A \textbf{non-coordinated} phase can be \emph{called} only while the sweep
        is inside its window, and is \textbf{forced off} at its \textbf{yield
        point} --- set back from the end of its window by that phase's own yellow
        and red clearance, so the \emph{next} phase goes green exactly as its own
        window begins. Min green always completes first, and a started pedestrian
        clearance always finishes.
  \item A \textbf{coordinated} phase is served every cycle with no call, has no
        max-out, and \emph{rests in green} whenever nothing else may run. It has a
        yield point of its own (\S\ref{sec:offsets}) and will not give up green
        before reaching it, even to waiting traffic.
  \item A split is the whole slice the movement owns --- \emph{green and its
        clearance}. Within that, the green is a \emph{maximum}, not a guarantee: a
        phase still gaps out early when traffic clears, and the unused remainder
        reverts to the coordinated phase.
\end{itemize}

\begin{warnbox}
Budget each split to hold its whole phase: minimum green (or walk plus pedestrian
clearance, whichever is longer), then yellow, then red clearance. A split too
short to contain its own phase makes the next phase start late on \emph{every}
cycle --- time no amount of offset correction can give back --- and the
controller says so once in the game log.
\end{warnbox}

\subsection{Splits are budgeted per barrier}\label{sec:splits}

The two rings cross a barrier together, so the cycle is divided barrier by
barrier, starting at local cycle 0. On each barrier, each ring's phases share
that barrier's time in ring-sequence order:
...
Two programming rules follow from this:

\begin{enumerate}[nosep]
  \item \textbf{Match each barrier's total across the rings.} In
        Figure~\ref{fig:ringsplit}, $\ph{5}+\ph{6} = \ph{2} = 50$ on barrier A
        and $\ph{8} = \ph{4} = 40$ on barrier B. If the totals differ, the engine
        gives the barrier the \emph{longer} total and stretches the shorter
        ring's phases to fill it, so they run longer than you typed. The game
        log warns you once.
  \item \textbf{Make the barriers total the cycle.} $50 + 40 = 90$. If they
        don't, every barrier is scaled by
        $\text{cycle} \div \text{barrier total}$ so the windows still tile the
        cycle (a split of 0 means ``an even share of its ring''). That is a
        safety net, not a feature: every split then runs a different length
        from what you typed.
\end{enumerate}

\begin{tipbox}
Sanity check: because the two rings time \emph{in parallel}, a ring with a phase
on every barrier has its own full cycle budget. On a 90\,s cycle, ring 1's splits
total 90 \textbf{and} ring 2's splits total 90 — so all eight splits together
total \textbf{180\,s, twice the cycle}. That is parallelism, not overbooking:
while \ph{2} owns 0--50 in ring 1, \ph{5} and \ph{6} split that same 0--50 in
ring 2. The R1/R2 tag beside each phase on the COORD screen shows its ring, and
A/B its barrier. If you instead spread the 90\,s across all eight phases, the
barriers total only 45, and scaling makes every green about \emph{twice} what
you typed.
\end{tipbox}

\paragraph{T intersections: a ring with nothing on a barrier.} A three-leg
intersection often has no ring-2 phase on the side-street barrier. Take
\ph{1}+\ph{2}+\ph{4} in ring 1 and only \ph{6} in ring 2. Ring 2 cannot total
the cycle, and it does not need to. It waits at the barrier while ring 1 serves
the side street, so \ph{6} takes only barrier A's time:

\begin{center}
\begin{tabular}{lllll}
\toprule
 & \multicolumn{2}{l}{Barrier A} & Barrier B & Cycle \\
\midrule
Ring 1 & \ph{1} = 10 & \ph{2} = 45 & \ph{4} = 35 & 90 \\
Ring 2 & \multicolumn{2}{l}{\ph{6} = 55 \quad ($= \ph{1}+\ph{2}$)} & (waits) & 55 \\
\bottomrule
\end{tabular}
\end{center}

Do \emph{not} set \ph{6} to 90 to make ring 2 ``total the cycle''. That makes
barrier A 90\,s long against 35\,s for barrier B, and the scaled cycle leaves
the side street about 25\,s. The flipped layout (\ph{2}+\ph{4} / \ph{5}+\ph{6})
works the same way: $\ph{2} = \ph{5}+\ph{6}$.
...
same moment as the called phase, holds green with it — the coordinated phases'
standing calls on the other barrier never strip it off early — and clears
\emph{with} it when the barrier's work is done; while riding along, the
companion's own split and force-off are ignored — they apply only when that phase
is served on its own call.
...
\subsubsection*{Getting back on offset}

Things knock a controller off its offset: a preemption parks normal operation, a
chunk reload rebuilds the ring engine, an operator drops the unit to flash and
back. The controller then has to walk its cycle back to where the master clock
says it should be.

It does so by \textbf{dwelling on the coordinated phase}. The coordinated phase
is the slack in the plan, so all correction is taken there:

\begin{itemize}[nosep]
  \item It will not surrender green \emph{early}. Until its yield point, gap-out
        and waiting cross traffic cannot end it --- it holds green, past max
        green, rather than giving time away and shifting the rest of the cycle.
  \item A coordinated phase that finds itself \emph{outside} its own window holds
        green until the yield point comes round again, absorbing the whole error
        in one long green.
  \item The one exception: outside its own window, it gives way when a waiting
        phase's window is \emph{about to open}. After an early return (the side
        street gapped out), the main street comes up early and would otherwise
        sit on a lead left's window at the top of the cycle, pushing the left to
        run after the side street instead.
\end{itemize}

Each non-coordinated phase is served at most once per window, so a side street on
recall that gaps out early gives its leftover time back instead of calling again.

Correction is therefore \textbf{add-only}: it lengthens the coordinated green and
never shortens a side street's split or a pedestrian interval to catch up. A
badly displaced controller is back in step within a cycle or two, having only
ever over-served the arterial.

\begin{tipbox}
While a controller is recovering, the coordinated green \emph{starts} late and
runs across the top of the cycle --- so ``phase 2 turned green at 0\,s'' is not
the test of a healthy plan. What matters is that the coordinated phase is
\textbf{green at} the offset, and that each side street \textbf{starts} at its own
split window. Both are true once the controller is in step.
\end{tipbox}
...
\begin{enumerate}[nosep]
  ...
  \item \field{Splits} — a value for every enabled phase (marked \texttt{*}).
        Match each barrier's total across the rings and check the barriers sum
        to the cycle; a ring with no phase on a barrier is left out of that
        barrier (\S\ref{sec:splits}). Size each
        split to hold the phase \emph{and} its clearance: min green (or walk plus
        ped clearance, whichever is longer), then yellow, then red clear.
...
(coordination already serves them every cycle); and WALK + ped clearance + yellow
+ red clear must fit inside each phase's split or that phase will push the next
one late on every cycle (safety intervals always finish). The controller writes a
one-line advisory to the game log naming any phase whose split cannot hold it.
...
Put a blankout box set to the \texttt{Train} legend on each approach to the
crossing, link it to that approach's circuit, and tick those circuits in
\field{SIGNS} (Section~\ref{sec:preemptsigns}): the signs flash for exactly as long
as the preempt runs, and are dark the rest of the time.
...
\begin{description}[nosep,leftmargin=4.2cm,style=nextline]
  ...
  \item[Dual entry (DE)] Serving an uncalled companion phase so both rings show
    green on the active barrier; the companion rides with the called phase and
    clears with it (cross-barrier demand never strips it off early).
...
\texttt{logic.RingBarrierState} & The per-tick dual-ring state machine: calls,
  greens, barriers, coordination windows, dual entry, conditional service. Yield
  points and the add-only dwell live here (\texttt{pastYieldPoint},
  \texttt{acceptanceOpen}, \texttt{coordYieldReached},
  \texttt{findSplitShortfall}). \\
