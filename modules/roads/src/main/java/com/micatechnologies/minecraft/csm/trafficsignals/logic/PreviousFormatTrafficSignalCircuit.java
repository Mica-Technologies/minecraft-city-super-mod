...
public class PreviousFormatTrafficSignalCircuit {
   ...
   */
  public PreviousFormatTrafficSignalCircuit() {
    // Do nothing
  }

  /**
   * Constructs a traffic signal circuit and populates its lists using the specified serialized
   * traffic signal circuit string.
   *
   * @param serialized serialized traffic signal circuit
   */
  public PreviousFormatTrafficSignalCircuit(String serialized) {
    // Split serialized string to lists
    String[] serializedLists = serialized.split(LIST_SEPARATOR_CHAR);

    // Process each serialized list
    for (int serializedListIndex = 0; serializedListIndex < serializedLists.length;
        serializedListIndex++) {
      if (serializedListIndex != LAST_SERVICE_TIME_INDEX) {
        // Split list into individual items
        String[] serializedListItems =
            serializedLists[serializedListIndex].split(ITEM_SEPARATOR_CHAR);
  ...
  /**
   * Returns a boolean value (true/false) indicating whether or not the specified {@link BlockPos}
   * is linked to this traffic signal circuit.
   *
  ...
  /**
   * Links the specified {@link BlockPos} to the traffic signal circuit as a hybrid left turn
   * signal.
   *
  ...
  /**
   * Links the specified {@link BlockPos} to the traffic signal circuit as a protected (bike, rail)
   * signal.
   *
   * @param blockPos block position of signal to link
   */
