 ...
 *
 * <p>The one ornament a player chooses, the finial, is its own block:
 * {@link BlockTrafficPoleFinial} sits in the air above a pole's top and brings its own collar, so
 * all the pole does about it is end its tube flush, exactly as it does under another pole. Six
 * ornaments on the pole's own blockstate would have multiplied its variants by seven.
 *
...
public class BlockTrafficPolePedestal extends AbstractBlockTrafficPole {
  ...
  private static EndStyle endStyle(IBlockAccess worldIn, BlockPos pos, EnumFacing facing,
    ...
    // A finial caps the tube with its own collar, which sleeves back over this block's tube end.
    // A cap under that collar would be a dome inside a casting, so the tube just stops.
    if (beyondBlock instanceof BlockTrafficPoleFinial) {
      return EndStyle.NONE;
    }
