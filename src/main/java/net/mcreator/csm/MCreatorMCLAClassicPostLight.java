...
public class MCreatorMCLAClassicPostLight extends Elementscsm.ModElement {
	@GameRegistry.ObjectHolder("csm:mclaclassicpostlight")
	public static final Block block = null;

	public MCreatorMCLAClassicPostLight(Elementscsm instance) {
		super(instance, 565);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0, new ModelResourceLocation("csm:mclaclassicpostlight", "inventory"));
	}
	...
	public static class BlockCustom extends Block {
		...
		public BlockCustom() {
			super(Material.ROCK);
			setRegistryName("mclaclassicpostlight");
			setUnlocalizedName("mclaclassicpostlight");
		...
		public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos) {
			switch ((EnumFacing) state.getValue(BlockDirectional.FACING)) {
				case SOUTH :
				default :
					return new AxisAlignedBB(1D, 0D, 1D, 0D, 1D, 0D);
				case NORTH :
					return new AxisAlignedBB(0D, 0D, 0D, 1D, 1D, 1D);
				case WEST :
					return new AxisAlignedBB(0D, 0D, 1D, 1D, 1D, 0D);
				case EAST :
					return new AxisAlignedBB(1D, 0D, 0D, 0D, 1D, 1D);
				case UP :
