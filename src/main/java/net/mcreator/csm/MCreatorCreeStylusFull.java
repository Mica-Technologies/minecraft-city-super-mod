...
public class MCreatorCreeStylusFull extends Elementscsm.ModElement {
	@GameRegistry.ObjectHolder("csm:creestylusfull")
	public static final Block block = null;

	public MCreatorCreeStylusFull(Elementscsm instance) {
		super(instance, 719);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0, new ModelResourceLocation("csm:creestylusfull",
				"inventory"));
	}
	...
	public static class BlockCustom extends Block {
		...
		public BlockCustom() {
			super(Material.ROCK);
			setRegistryName("creestylusfull");
			setUnlocalizedName("creestylusfull");
			setSoundType(SoundType.STONE);
			setHardness(1F);
			setResistance(10F);
			setLightLevel(0F);
			setLightOpacity(0);
			setCreativeTab(MCreatorMCLAElectricTab.tab);
		...
		public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos) {
			switch ((EnumFacing) state.getValue(BlockDirectional.FACING)) {
				case SOUTH :
				default :
					return new AxisAlignedBB(1D, 0.5D, 0D, 0D, 1D, 2D);
				case NORTH :
					return new AxisAlignedBB(0D, 0.5D, 1D, 1D, 1D, -1D);
				case WEST :
					return new AxisAlignedBB(1D, 0.5D, 1D, -1D, 1D, 0D);
				case EAST :
					return new AxisAlignedBB(0D, 0.5D, 0D, 2D, 1D, 1D);
				case UP :
					return new AxisAlignedBB(0D, 0D, 0.5D, 1D, 2D, 1D);
				case DOWN :
					return new AxisAlignedBB(0D, 1D, 0.5D, 1D, -1D, 0D);
			}
		}
