...
public class CitySuperModVariables {
	public static class MapVariables extends WorldSavedData {
		public static final String DATA_NAME = "csm_mapvars";
		public MapVariables() {
			super(DATA_NAME);
		}
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				CitySuperMod.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(0, this));
			} else {
				CitySuperMod.PACKET_HANDLER.sendToAll(new WorldSavedDataSyncMessage(0, this));
			}
		}
	...
	public static class WorldVariables extends WorldSavedData {
		public static final String DATA_NAME = "csm_worldvars";
		public double currentPanelX = 0;
		public double currentPanelY = 0;
		public double currentPanelZ = 0;
		public String currentPanelMap = "";
		...
		@Override
		public void readFromNBT(NBTTagCompound nbt) {
			currentPanelX = nbt.getDouble("currentPanelX");
			currentPanelY = nbt.getDouble("currentPanelY");
			currentPanelZ = nbt.getDouble("currentPanelZ");
			currentPanelMap = nbt.getString("currentPanelMap");
		}

		@Override
		public NBTTagCompound writeToNBT(NBTTagCompound nbt) {
			nbt.setDouble("currentPanelX", currentPanelX);
			nbt.setDouble("currentPanelY", currentPanelY);
			nbt.setDouble("currentPanelZ", currentPanelZ);
			nbt.setString("currentPanelMap", currentPanelMap);
			return nbt;
		}
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				CitySuperMod.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(1, this));
			} else {
				CitySuperMod.PACKET_HANDLER.sendToDimension(new WorldSavedDataSyncMessage(1, this), world.provider.getDimension());
			}
		}
	...
	public static class WorldSavedDataSyncMessageHandler implements IMessageHandler<WorldSavedDataSyncMessage, IMessage> {
		...
		private void syncData(WorldSavedDataSyncMessage message, MessageContext context, World world) {
			if (context.side == Side.SERVER) {
				...
				if (message.type == 0)
					CitySuperMod.PACKET_HANDLER.sendToAll(message);
				else
					CitySuperMod.PACKET_HANDLER.sendToDimension(message, world.provider.getDimension());
			}
			if (message.type == 0) {
