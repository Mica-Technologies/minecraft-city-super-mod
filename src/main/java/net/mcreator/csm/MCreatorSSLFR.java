...
public class MCreatorSSLFR extends Elementscsm.ModElement {
	@GameRegistry.ObjectHolder("csm:sslfr")
	public static final Block block = null;

	public MCreatorSSLFR(Elementscsm instance) {
		super(instance, 737);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0, new ModelResourceLocation("csm:sslfr", "inventory"));
	}
	...
	public static class BlockCustom extends Block {
		...
		public BlockCustom() {
			super(Material.ROCK);
			setRegistryName("sslfr");
			setUnlocalizedName("sslfr");
		...
		public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos) {
			switch ((EnumFacing) state.getValue(BlockDirectional.FACING)) {
				case SOUTH :
				default :
					return new AxisAlignedBB(1D, 0.5D, 1D, 0D, 1D, 2D);
				case NORTH :
					return new AxisAlignedBB(0D, 0.5D, 0D, 1D, 1D, -1D);
				case WEST :
					return new AxisAlignedBB(0D, 0.5D, 1D, -1D, 1D, 0D);
				case EAST :
					return new AxisAlignedBB(1D, 0.5D, 0D, 2D, 1D, 1D);
				case UP :
					return new AxisAlignedBB(0D, 1D, 0.5D, 1D, 2D, 1D);
				case DOWN :
