...
public class BlockRedstoneTTSGui extends GuiScreen
{
    ...
    public void initGui() {
        ...
        this.ttsStringField = new GuiMultiLineTextField( 0, fontRenderer, sr.getScaledWidth() / 2 - 100, 5, 200, 150 );
        this.ttsVoiceDurationStretchField = new GuiTextField( 1, fontRenderer, sr.getScaledWidth() / 2 - 100,
                                                              sr.getScaledHeight() - 85, 200, 25 );
        this.buttonList.add(
                closeButton = new GuiButton( 0, sr.getScaledWidth() / 2 - 100, sr.getScaledHeight() - 50, 200, 20,
                                             "Close" ) );
        this.buttonList.add(
                cancelButton = new GuiButton( 1, sr.getScaledWidth() / 2 - 100, sr.getScaledHeight() - 25, 200, 20,
                                              "Cancel" ) );
        ...
        if ( button == closeButton ) {
            try {
                tileEntityRedstoneTTS.setTtsString( ttsStringField.getText()
                                                                  ...
                                                                  .replaceAll( "\n", " " )
                                                                  .replaceAll( "\r", " " ) );
                //tileEntityRedstoneTTS.setTtsVoiceDurationStretch( Float.parseFloat(
                ////        ttsVoiceDurationStretchField.getText()
                ///                                    .replaceAll( "\r\n", " " )
                //                                    .replaceAll( "\n", " " )
                 //                                   .replaceAll( "\r", " " ) ) );
            }
