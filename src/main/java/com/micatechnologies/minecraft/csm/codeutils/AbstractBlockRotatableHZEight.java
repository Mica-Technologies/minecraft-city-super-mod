...
public abstract class AbstractBlockRotatableHZEight extends AbstractBlock {
  ...
  public IBlockState getStateForPlacement(World worldIn, BlockPos pos, EnumFacing facing,
    ...
    DirectionEight direction = getDirectionFromFacingAndHit(placer.getHorizontalFacing().getOpposite(), hitX);
  ...
  /**
   * Gets the {@link DirectionEight} from the specified {@link EnumFacing} and hit coordinates.
   *
   * @param facing the {@link EnumFacing} of the block
   * @param hitX   the X coordinate of the hit
   *
   * @return
   */

  private DirectionEight getDirectionFromFacingAndHit(EnumFacing facing, float hitX) {
    boolean isCenter = hitX > 0.25 && hitX < 0.75;

    switch (facing) {
      case NORTH:
        return isCenter ? DirectionEight.N : (hitX < 0.5 ? DirectionEight.NW : DirectionEight.NE);
      case SOUTH:
        return isCenter ? DirectionEight.S : (hitX < 0.5 ? DirectionEight.SW : DirectionEight.SE);
      case WEST:
        return isCenter ? DirectionEight.W : (hitX < 0.5 ? DirectionEight.SW : DirectionEight.NW);
      case EAST:
        return isCenter ? DirectionEight.E : (hitX < 0.5 ? DirectionEight.SE : DirectionEight.NE);
      default:
        return DirectionEight.N; // Default to North for any unexpected cases
    }
  }
