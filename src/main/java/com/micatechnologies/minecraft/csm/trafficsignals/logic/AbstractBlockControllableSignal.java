...
public abstract class AbstractBlockControllableSignal extends Block
{
    ...
    public static void changeSignalColor( World world, BlockPos blockPos, int signalColor ) {
        ...
        if ( blockState.getBlock() instanceof AbstractBlockTrafficSignalRequester ) {
            // Reset request count if entering/leaving walk phase (green is walk phase, yellow is end of walk phase)
    ...
    public static AbstractBlockControllableSignal getSignalBlockInstanceOrNull( World world, BlockPos blockPos ) {
        ...
        try {
            IBlockState blockState = world.getBlockState( blockPos );
            if ( blockState.getBlock() instanceof AbstractBlockControllableSignal ) {
                return ( AbstractBlockControllableSignal ) blockState.getBlock();
            }
        }
        catch ( Exception e ) {
            System.err.println( "Error getting signal block instance at " + blockPos + ": " + e.getMessage() );
            return null;
        }
        return null;
    }
