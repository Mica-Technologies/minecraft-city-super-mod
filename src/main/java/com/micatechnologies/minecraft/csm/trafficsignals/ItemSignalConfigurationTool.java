...
public class ItemSignalConfigurationTool extends ElementsCitySuperMod.ModElement
{
    @GameRegistry.ObjectHolder( "csm:signalconfigurationtool" )
    public static final Item block = null;

    public ItemSignalConfigurationTool( ElementsCitySuperMod instance ) {
        super( instance, 2138 );
    }
    ...
    public static class ItemCustom extends Item
    {
        ...
        public ItemCustom() {
            setMaxDamage( 0 );
            maxStackSize = 1;
            setUnlocalizedName( "signalconfigurationtool" );
            setRegistryName( "signalconfigurationtool" );
                                           ...
                                           float hitZ )
        {
            if ( !worldIn.isRemote ) {
                ...
                if ( player.isSneaking() ) {
                    ...
                    // Notify player
                    player.sendMessage( new TextComponentString( "Mode changed to: " + getMode( player ).getFriendlyName() ) );
                ...
                // If in clear controller faults mode, clear controller faults if clicked block is a controller
                    ...
                    if ( tileEntity instanceof TileEntityTrafficSignalController ) {
                        ...
                        if ( isInFaultState ) {
                            player.sendMessage(
                                    new TextComponentString( "Controller fault state has been reset." ) );
                            player.sendMessage( new TextComponentString(
                                    "Cleared controller fault message: " + faultMessage ) );
                        }
                        // Controller in non-fault state
                        else {
                            player.sendMessage( new TextComponentString(
                                    "Controller is not in fault state. Unable to reset!" ) );
                        }
                    ...
                    if ( clickedBlock instanceof BlockTrafficSignalController.BlockCustom ) {
                    ...
                    // Check if clicked block is a signal
                        ...
                        // Otherwise, create overlap on selected controller using source and target signal positions
                            ...
                            if ( tileEntity instanceof TileEntityTrafficSignalController ) {
                                ...
                                // Create overlap
                                boolean created = tileEntityTrafficSignalController.createOverlap( overlapSourcePos,
                                                                                                   pos );
                    ...
                    // If clicked block is not a signal or controller, reset pos1 and pos2
                        ...
                        // Notify player
                        player.sendMessage( new TextComponentString( "Overlap source signal position " +
                                                                             "has been reset. (Clicked non-signal " +
                                                                             "block)" ) );
