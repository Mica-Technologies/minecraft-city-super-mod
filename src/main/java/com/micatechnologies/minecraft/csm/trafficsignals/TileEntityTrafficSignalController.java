...
public class TileEntityTrafficSignalController extends AbstractTickableTileEntity
{
     ...
     * @since 2.0
     */
    private static final String NBT_KEY_LAST_PEDESTRIAN_PHASE_TIME = "tcLastPedPhaseTime";
     ...
     * @since 2.0
     */
    private static final String NBT_KEY_OVERLAP_PEDESTRIAN_SIGNALS = "tcOverlapPedestrianSignals";
    ...
    /**
     * The key for storing and retrieving the traffic signal controller's minimum green time (milliseconds) setting from
     * NBT data.
     *
     * @since 2.0
     */
    private static final String NBT_KEY_MIN_GREEN_TIME_MS = "tcMinGreenTimeMs";

    /**
     * The key for storing and retrieving the traffic signal controller's maximum green time (milliseconds) setting from
     * NBT data.
     *
     * @since 2.0
     */
    private static final String NBT_KEY_MAX_GREEN_TIME_MS = "tcMaxGreenTimeMs";

    /**
     * The key for storing and retrieving the traffic signal controller's secondary minimum green time (milliseconds)
     * setting from NBT data.
     *
     * @since 2.0
     */
    private static final String NBT_KEY_MIN_GREEN_TIME_SECONDARY_MS = "tcMinGreenSecondaryTimeMs";

    /**
     * The key for storing and retrieving the traffic signal controller's secondary maximum green time (milliseconds)
     * setting from NBT data.
     *
     * @since 2.0
     */
    private static final String NBT_KEY_MAX_GREEN_TIME_SECONDARY_MS = "tcMaxGreenSecondaryTimeMs";
    ...
    /**
     * The minimum green time (milliseconds) when servicing circuits configured to the traffic signal controller in
     * {@link TrafficSignalControllerMode#NORMAL} mode.
     *
     * @since 2.0
     */
    private long minGreenTimeMs = 20000L;

    /**
     * The maximum green time (milliseconds) when servicing circuits configured to the traffic signal controller in
     * {@link TrafficSignalControllerMode#NORMAL} mode.
     *
     * @since 2.0
     */
    private long maxGreenTimeMs = 120000L;

    /**
     * The secondary minimum green time (milliseconds) when servicing circuits configured to the traffic signal
     * controller in {@link TrafficSignalControllerMode#NORMAL} mode.
     *
     * @since 2.0
     */
    private long minGreenTimeSecondaryMs = 10000L;

    /**
     * The secondary maximum green time (milliseconds) when servicing circuits configured to the traffic signal
     * controller in {@link TrafficSignalControllerMode#NORMAL} mode.
     *
     * @since 2.0
     */
    private long maxGreenTimeSecondaryMs = 60000L;
    ...
    public void onTick() {
        ...
        try {
            ...
            TrafficSignalPhase newPhase = TrafficSignalControllerTicker.tick( getWorld(), mode, operatingMode, circuits,
                                                                              ...
                                                                              alternatingFlash,
                                                                              overlapPedestrianSignals, yellowTimeMs,
                                                                              flashDontWalkTimeMs, allRedTimeMs,
                                                                              minRequestableServiceTimeMs,
                                                                              maxRequestableServiceTimeMs,
                                                                              minGreenTimeMs, maxGreenTimeMs,
                                                                              minGreenTimeSecondaryMs,
            ...
            if ( newPhase != null ) {
                ...
                lastPhaseChangeTime = getWorld().getTotalWorldTime();
    ...
    public NBTTagCompound writeNBT( NBTTagCompound compound ) {
        ...
        compound.setLong( NBT_KEY_LAST_PEDESTRIAN_PHASE_TIME, lastPedPhaseTime );
        ...
        compound.setBoolean( NBT_KEY_OVERLAP_PEDESTRIAN_SIGNALS, overlapPedestrianSignals );
        ...
        // Write the minimum green time (ms) to NBT
        compound.setLong( NBT_KEY_MIN_GREEN_TIME_MS, minGreenTimeMs );

        // Write the maximum green time (ms) to NBT
        compound.setLong( NBT_KEY_MAX_GREEN_TIME_MS, maxGreenTimeMs );

        // Write the secondary minimum green time (ms) to NBT
        compound.setLong( NBT_KEY_MIN_GREEN_TIME_SECONDARY_MS, minGreenTimeSecondaryMs );

        // Write the secondary maximum green time (ms) to NBT
        compound.setLong( NBT_KEY_MAX_GREEN_TIME_SECONDARY_MS, maxGreenTimeSecondaryMs );
    ...
    public void readNBT( NBTTagCompound compound ) {
        ...
        if ( compound.hasKey( NBT_KEY_LAST_PEDESTRIAN_PHASE_TIME ) ) {
            lastPedPhaseTime = compound.getLong( NBT_KEY_LAST_PEDESTRIAN_PHASE_TIME );
        }
        ...
        if ( compound.hasKey( NBT_KEY_OVERLAP_PEDESTRIAN_SIGNALS ) ) {
            overlapPedestrianSignals = compound.getBoolean( NBT_KEY_OVERLAP_PEDESTRIAN_SIGNALS );
        }
        ...
        // Load the traffic signal controller minimum green time (ms)
        if ( compound.hasKey( NBT_KEY_MIN_GREEN_TIME_MS ) ) {
            minGreenTimeMs = compound.getLong( NBT_KEY_MIN_GREEN_TIME_MS );
        }

        // Load the traffic signal controller maximum green time (ms)
        if ( compound.hasKey( NBT_KEY_MAX_GREEN_TIME_MS ) ) {
            maxGreenTimeMs = compound.getLong( NBT_KEY_MAX_GREEN_TIME_MS );
        }

        // Load the traffic signal controller secondary minimum green time (ms)
        if ( compound.hasKey( NBT_KEY_MIN_GREEN_TIME_SECONDARY_MS ) ) {
            minGreenTimeSecondaryMs = compound.getLong( NBT_KEY_MIN_GREEN_TIME_SECONDARY_MS );
        }

        // Load the traffic signal controller secondary maximum green time (ms)
        if ( compound.hasKey( NBT_KEY_MAX_GREEN_TIME_SECONDARY_MS ) ) {
            maxGreenTimeSecondaryMs = compound.getLong( NBT_KEY_MAX_GREEN_TIME_SECONDARY_MS );
        }
    }
