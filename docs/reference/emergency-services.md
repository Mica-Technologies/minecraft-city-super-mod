# Emergency Services

Fire, police and ambulance station fittings, community warning sirens and dispatch.

!!! info "62 blocks in this tab"

    Every block below is registered in the mod and has a blockstate on disk. This page is
    generated from the source, so it cannot fall behind what the mod actually ships.

<div class="block-table" markdown>

| Block | Registry ID | Hardness | Resistance | Tool | Harvest |
|---|---|---|---|---|---|
| 911 Dispatch Console | `csm:dispatch_console` | 2.0 | 6.0 | pickaxe | 0 |
| Air Compressor | `csm:air_compressor` | 2.0 | 6.0 | pickaxe | 0 |
| Ambulance Stretcher | `csm:ems_stretcher` | 2.0 | 6.0 | pickaxe | 0 |
| Ambulance Stretcher (Lowered) | `csm:ems_stretcher_lowered` | 2.0 | 6.0 | pickaxe | 0 |
| Backboard Wall Rack | `csm:ems_backboard_rack` | 2.0 | 6.0 | pickaxe | 0 |
| Bay Door Clearance Light | `csm:bay_clearance_light` | 2.0 | 6.0 | pickaxe | 0 |
| Blue Light Emergency Call Box | `csm:blue_light_call_box` | 2.0 | 6.0 | pickaxe | 0 |
| Booking Camera | `csm:booking_camera` | 2.0 | 6.0 | pickaxe | 0 |
| Booking Height Chart | `csm:height_chart` | 2.0 | 6.0 | pickaxe | 0 |
| Controlled Medication Safe | `csm:medication_safe` | 2.0 | 6.0 | pickaxe | 0 |
| Decontamination Sink | `csm:decon_sink` | 2.0 | 6.0 | pickaxe | 0 |
| Electronic Outdoor Warning Siren | `csm:warning_siren_electronic` | 2.0 | 6.0 | pickaxe | 0 |
| Emergency Eyewash Station | `csm:eyewash_station` | 2.0 | 6.0 | pickaxe | 0 |
| Equipment Locker | `csm:equipment_locker` | 2.0 | 6.0 | pickaxe | 0 |
| Evacuation Assembly Point Sign | `csm:assembly_point_sign` | 2.0 | 6.0 | pickaxe | 0 |
| Evidence Locker | `csm:evidence_locker` | 2.0 | 6.0 | pickaxe | 0 |
| Fallout Shelter Sign | `csm:fallout_shelter_sign` | 2.0 | 6.0 | pickaxe | 0 |
| Fingerprint Scanner | `csm:fingerprint_scanner` | 2.0 | 6.0 | pickaxe | 0 |
| Fire Line Tape | `csm:fire_line_tape` | 0.2 | 0.2 | shears | 0 |
| Fire Pole | `csm:fire_pole` | 2.0 | 6.0 | pickaxe | 0 |
| Fire Pole Floor Opening | `csm:fire_pole_hole` | 2.0 | 6.0 | pickaxe | 0 |
| Firefighting Tool Board | `csm:fire_tool_board` | 2.0 | 6.0 | pickaxe | 0 |
| Firehouse Alarm Gong | `csm:station_alarm_gong` | 2.0 | 6.0 | pickaxe | 0 |
| First Aid Cabinet | `csm:first_aid_cabinet` | 2.0 | 6.0 | pickaxe | 0 |
| Front Desk Counter | `csm:front_desk_counter` | 2.0 | 6.0 | pickaxe | 0 |
| Holding Cell Bench | `csm:holding_cell_bench` | 2.0 | 6.0 | pickaxe | 0 |
| Holding Cell Door | `csm:holding_cell_door` | 2.0 | 6.0 | pickaxe | 0 |
| Holding Cell Toilet and Sink | `csm:holding_cell_toilet` | 2.0 | 6.0 | pickaxe | 0 |
| Hose Drying Rack | `csm:hose_drying_rack` | 2.0 | 6.0 | pickaxe | 0 |
| K-9 Kennel | `csm:k9_kennel` | 2.0 | 6.0 | pickaxe | 0 |
| Lobby Courtesy Phone | `csm:lobby_phone` | 2.0 | 6.0 | pickaxe | 0 |
| Maltese Cross Emblem | `csm:maltese_cross_emblem` | 2.0 | 6.0 | pickaxe | 0 |
| Medical Supply Shelving | `csm:ems_supply_shelving` | 2.0 | 6.0 | pickaxe | 0 |
| Nozzle Rack | `csm:nozzle_rack` | 2.0 | 6.0 | pickaxe | 0 |
| Oxygen Cylinder Rack | `csm:oxygen_cylinder_rack` | 2.0 | 6.0 | pickaxe | 0 |
| Pass-Through Deal Tray | `csm:pass_through_tray` | 2.0 | 6.0 | pickaxe | 0 |
| Police Line Tape | `csm:police_line_tape` | 0.2 | 0.2 | shears | 0 |
| Police Star Emblem | `csm:police_star_emblem` | 2.0 | 6.0 | pickaxe | 0 |
| Police Station Blue Lamp | `csm:police_lamp` | 2.0 | 6.0 | pickaxe | 0 |
| Property Bin Shelf | `csm:property_bins` | 2.0 | 6.0 | pickaxe | 0 |
| Radio Console Speaker | `csm:radio_console_speaker` | 2.0 | 6.0 | pickaxe | 0 |
| Rolled Fire Hose | `csm:hose_rolls` | 2.0 | 6.0 | pickaxe | 0 |
| Rotating Outdoor Warning Siren | `csm:warning_siren_rotating` | 2.0 | 6.0 | pickaxe | 0 |
| SCBA Cylinder Cascade | `csm:scba_cylinder_cascade` | 2.0 | 6.0 | pickaxe | 0 |
| SCBA Fill Station | `csm:scba_fill_station` | 2.0 | 6.0 | pickaxe | 0 |
| SCBA Wall Rack | `csm:scba_wall_rack` | 2.0 | 6.0 | pickaxe | 0 |
| Stair Chair | `csm:ems_stair_chair` | 2.0 | 6.0 | pickaxe | 0 |
| Star of Life Emblem | `csm:star_of_life_emblem` | 2.0 | 6.0 | pickaxe | 0 |
| Station Alert Light (Red) | `csm:station_alert_light_red` | 2.0 | 6.0 | pickaxe | 0 |
| Station Alert Light (White) | `csm:station_alert_light_white` | 2.0 | 6.0 | pickaxe | 0 |
| Station Alerting Controller | `csm:station_alert_controller` | 2.0 | 6.0 | pickaxe | 0 |
| Station Alerting Relay | `csm:station_alert_relay` | 2.0 | 6.0 | pickaxe | 0 |
| Station Alerting Speaker | `csm:station_alert_speaker` | 2.0 | 6.0 | pickaxe | 0 |
| Station Number Plaque | `csm:station_number_plaque` | 2.0 | 6.0 | pickaxe | 0 |
| Storm Shelter Sign | `csm:storm_shelter_sign` | 2.0 | 6.0 | pickaxe | 0 |
| Tape Stanchion | `csm:tape_stanchion` | 2.0 | 6.0 | pickaxe | 0 |
| Turnout Gear Extractor | `csm:gear_extractor` | 2.0 | 6.0 | pickaxe | 0 |
| Turnout Gear Locker | `csm:turnout_gear_locker` | 2.0 | 6.0 | pickaxe | 0 |
| Turnout Gear Locker (Empty) | `csm:turnout_gear_locker_empty` | 2.0 | 6.0 | pickaxe | 0 |
| Walk-Through Metal Detector | `csm:metal_detector` | 2.0 | 6.0 | pickaxe | 0 |
| Wall Hose Rack (Loaded) | `csm:hose_rack_wall` | 2.0 | 6.0 | pickaxe | 0 |
| Warning Siren Controller | `csm:warning_siren_controller` | 2.0 | 6.0 | pickaxe | 0 |

</div>
