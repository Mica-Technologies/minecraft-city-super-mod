...
!!! note "Compile the integrity tool before running it"
    ...
    tree reports **0 errors**, so any error it prints is worth investigating. It also reports 236
...
| `gen_official_faces.py` | Swaps an existing road sign's face for its SHS drawing (or a drawn face where none exists), texture only, the plate read from the sign's own blockstate; legends in the real FHWA series; `--sheet` for the before/after review, `--check` for drift |
| `detect_legend_series.py` | Measures a sign's original texture: each legend line's position, cap height and FHWA series |
